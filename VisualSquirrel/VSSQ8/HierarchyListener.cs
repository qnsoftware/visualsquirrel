...
namespace Squirrel.SquirrelLanguageService
{
    ...
    internal class HierarchyListener : IVsHierarchyEvents, IDisposable
    {
        ...
        public int OnItemAdded(uint itemidParent, uint itemidSiblingPrev, uint itemidAdded)
        {
            ...
            Debug.WriteLine("\n\tOnItemAdded\n");
            string name;
            if (!IsSquirrelFile(itemidAdded, out name))
            {
                return VSConstants.S_OK;
            }
            ...
            if (null != onItemAdded)
            {
                HierarchyEventArgs args = new HierarchyEventArgs(itemidAdded, name, sqVersion);
        ...
        public int OnItemDeleted(uint itemid)
        {
            ...
            string name;
            if (!IsSquirrelFile(itemid, out name))
            {
                return VSConstants.S_OK;
            }
            if (null != onItemDeleted)
            {
                HierarchyEventArgs args = new HierarchyEventArgs(itemid, name, sqVersion);
        ...
        private bool IsSquirrelFile(uint itemId, out string canonicalName)
        {
            ...
            return (0 == string.Compare(extension, SQLanguageService.LanguageExtension, StringComparison.OrdinalIgnoreCase));
        }
        ...
        private void InternalScanHierarchy(uint itemId)
        {
            ...
            while (VSConstants.VSITEMID_NIL != currentItem)
            {
                ...
                string itemName;
                if ((null != onItemAdded) && IsSquirrelFile(currentItem, out itemName))
                {
                    HierarchyEventArgs args = new HierarchyEventArgs(currentItem, itemName, sqVersion);
                    onItemAdded(hierarchy, args);
                }
